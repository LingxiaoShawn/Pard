...
class SpectreSamplingMetrics:
    def __init__(self, original_loaders, compute_emd=False, metrics_list=['degree', 'clustering', 'orbit']):
        super().__init__()

        self.train_graphs = self.loader_to_nx(original_loaders['train'])
        self.val_graphs = self.loader_to_nx(original_loaders['val'])
        self.test_graphs = self.loader_to_nx(original_loaders['test'])
    ...
    def __call__(self, generated_graphs: list, local_rank=0, test=True):
...
class EMDSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, original_loaders):
        super().__init__(original_loaders=original_loaders,
                         compute_emd=True,
                         metrics_list=['degree', 'clustering', 'orbit'])

class MMDSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, original_loaders):
        super().__init__(original_loaders=original_loaders,
                         compute_emd=False,
                         metrics_list=['degree', 'clustering', 'orbit'])
