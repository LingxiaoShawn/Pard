...
class StructureFeatures:
    def __init__(self, extra_features_type='all', k_eigval=5, k_eigvec=2):
        ...
        if extra_features_type in ['eigenvalues', 'all']:
            self.eigenfeatures = EigenFeatures(mode=extra_features_type, k_eigval=self.k_eigval, k_eigvec=self.k_eigvec)
        
    def calculate_dims(self):
        base_dim_x, base_dim_y = 0, 0 
        if self.features_type == 'cycles':
            return base_dim_x + 3, base_dim_y + 4
        elif self.features_type == 'eigenvalues':
            return base_dim_x + 3, base_dim_y + 4 + self.k_eigval + 1
        elif self.features_type == 'all':
            return base_dim_x + 3 + 1 + self.k_eigvec, base_dim_y + 4 + 1 + self.k_eigval
        else:
            raise ValueError(f"Features type {self.features_type} not implemented")

    def __call__(self, X_0, E, node_mask):
        X_0 = X_0 * node_mask.unsqueeze(-1)
        E = E * (node_mask.unsqueeze(-2)[..., None] * node_mask.unsqueeze(-1)[..., None]).type_as(E)
        X, Y = self.ncycles(E, node_mask)        # (bs, n, 3), (bs, 4)
        if self.features_type in ['eigenvalues', 'all']:
            eigenfeatures = self.eigenfeatures(E, node_mask)
            if self.features_type == 'eigenvalues':
                n_components, batched_eigenvalues = eigenfeatures  # (bs, 1), (bs, k1)
            else:
                n_components, batched_eigenvalues, nonlcc_indicator, k_lowest_eigvec = eigenfeatures   # (bs, 1), (bs, k1),
                                                                                                       # (bs, n, 1), (bs, n, k2)
                X = torch.cat((X, nonlcc_indicator, k_lowest_eigvec), dim=-1)
            Y = torch.hstack((Y, n_components, batched_eigenvalues))
        return X, Y  
...
class EigenFeatures:
    ...
    """
    def __init__(self, mode='all', k_eigval=5, k_eigvec=2):
    ...
    def __call__(self, E, node_mask):
        ...
        if self.mode == 'eigenvalues':
            ...
            n_connected_comp, batch_eigenvalues = get_eigenvalues_features(eigenvalues=eigvals, k=self.k_eigval)
